const request = require('supertest');
const Database = require('better-sqlite3');
const { createSchema } = require('../src/database/schema');
const path = require('path');
const fs = require('fs');

let mockDb;

jest.mock('../src/database/db', () => ({
  getDb: () => mockDb,
  closeDb: () => {}
}));

const { createApp } = require('../src/app');

let app;
let sessionToken;

// Minimal valid JPEG buffer (smallest valid JPEG)
function createMinimalJpeg() {
  return Buffer.from([
    
  ]);
}

beforeEach(() => {
  mockDb = new Database(':memory:');
  mockDb.pragma('journal_mode = WAL');
  mockDb.pragma('foreign_keys = ON');
  createSchema(mockDb);
  app = createApp();

  // Create a guest session
  const token = require('crypto').randomBytes(32).toString('hex');
  mockDb.prepare(
    'INSERT INTO sessions (session_token, customer_id, is_guest) VALUES (?, ?, ?)'
  ).run(token, null, 1);
  sessionToken = token;
});

afterEach(() => {
  mockDb.close();
});

describe('POST /api/classify', () => {
  test('classifies a valid JPEG image', async () => {
    const jpeg = createMinimalJpeg();
    const res = await request(app)
      .post('/api/classify')
      .set('X-Session-Token', sessionToken)
      .attach('image', jpeg, { filename: 'test.jpg', contentType: 'image/jpeg' });

    expect(res.status).toBe(200);
    expect(res.body.waste_type).toBeDefined();
    expect(res.body.confidence).toBeGreaterThanOrEqual(0.6);
    expect(res.body.container).toBeDefined();
    expect(res.body.label).toBeDefined();
  });

  test('returns 401 without session token', async () => {
    const jpeg = createMinimalJpeg();
    const res = await request(app)
      .post('/api/classify')
      .attach('image', jpeg, { filename: 'test.jpg', contentType: 'image/jpeg' });

    expect(res.status).toBe(401);
  });

  test('returns 401 with invalid session token', async () => {
    const jpeg = createMinimalJpeg();
    const res = await request(app)
      .post('/api/classify')
      .set('X-Session-Token', 'totally-invalid-token')
      .attach('image', jpeg, { filename: 'test.jpg', contentType: 'image/jpeg' });

    expect(res.status).toBe(401);
  });

  test('returns 400 for invalid file format (text file)', async () => {
    const textBuffer = Buffer.from('this is not an image file at all');
    const res = await request(app)
      .post('/api/classify')
      .set('X-Session-Token', sessionToken)
      .attach('image', textBuffer, { filename: 'test.txt', contentType: 'text/plain' });

    expect(res.status).toBe(400);
  });

  test('returns 413 for file too large', async () => {
    const largeBuffer = Buffer.alloc(6 * 1024 * 1024, 0xff); // 6MB, starts with 0xff
    // Make it look like JPEG
    largeBuffer[0] = 0xff;
    largeBuffer[1] = 0xd8;
    largeBuffer[2] = 0xff;

    const res = await request(app)
      .post('/api/classify')
      .set('X-Session-Token', sessionToken)
      .attach('image', largeBuffer, { filename: 'large.jpg', contentType: 'image/jpeg' });

    expect(res.status).toBe(413);
  });

  test('returns 400 when no image is provided', async () => {
    const res = await request(app)
      .post('/api/classify')
      .set('X-Session-Token', sessionToken)
      .send({});

    expect(res.status).toBe(400);
    expect(res.body.code).toBe('NO_IMAGE');
  });

  test('guest session does not earn points', async () => {
    const jpeg = createMinimalJpeg();
    const res = await request(app)
      .post('/api/classify')
      .set('X-Session-Token', sessionToken)
      .attach('image', jpeg, { filename: 'test.jpg', contentType: 'image/jpeg' });

    expect(res.status).toBe(200);
    expect(res.body.points_awarded).toBe(0);
    expect(res.body.customer).toBeNull();
  });
});
